...
void AlgoritmoDeHuffman(string* vetAux, int grupo, double *vetDados) {
    ...
    for (int i = 0; i < grupo; i++) {
        tamanhoCodigo = trie->ConstroiHuffmanTrie(vetAux[i], vetAux[i].length());
        taxaDeComp = taxaDeComp + (tamanhoCodigo / vetAux[i].length() * 64);
        armazen = armazen + (tamanhoCodigo / 8);
    }
    vetDados[0] = vetDados[0] + (double)(taxaDeComp / grupo);
    vetDados[1] = vetDados[1] + (double)(armazen / grupo);
}

void AlgoritmoLZW(string* vetAux, int grupo, double *vetDados) {
    vector<int> codigo;
    double taxaDeComp = 0.0;
    double armazen = 0.0;
    for (int i = 0; i < grupo; i++) {
        codigo = codificacaoLZW(vetAux[i]);
        taxaDeComp = taxaDeComp + ((double)codigo.size() / (double)vetAux[i].length());
...
void medias2(int grupo, double *vetDados, string *vetConj){
    ...
    for(int i = 0; i < 5; i++){            //ele deve fazer isso 5 vezes, como pedido
        ...
        while(c < grupo) {                 //preenche o vetor com valores aleatorios referente a entrada
            ...
            trocaString(&vetConj[r], &vetConj[17061 - c]);    // no vetConj, com o ultimo elemento do vetor e geramos um numero aleatorio entre 0 e 17062 - c,
            c++;                                           // para q nao coletemos o mesmo valor uma outra vez
            vetAux[c] = vetConj[c];
            c++;
        }
        auto t1 = std::chrono::high_resolution_clock::now();    //comeca a contar o tempo
        cout << "Comprimindo Huffman..." << endl;
        AlgoritmoDeHuffman(vetAux, grupo, vetDados);
        cout << "Compressao Huffman feita com sucesso!" << endl << endl;
