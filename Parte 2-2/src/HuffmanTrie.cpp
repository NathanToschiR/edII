...
int HuffmanTrie::AchaNoVet(char letra, int n, NoTrie* vet) {
    for (int i = 0; i < n; i++) {
        if (vet[i].getLetra() == letra)
            return i;
    }
    return 0;
}

void HuffmanTrie::DefineCodigos(NoTrie* p, string str, int n, NoTrie* vet) {
    if (p->getFilhoEsq() == NULL) {
        int aux = AchaNoVet(p->getLetra(), n, vet);
        vet[aux].setHuff(str);
    }
    else {
        DefineCodigos(p->getFilhoEsq(), str + '0', n, vet);
        DefineCodigos(p->getFilhoDir(), str + '1', n, vet);
    }
}

int HuffmanTrie::ConstroiHuffmanTrie(string msg, int n) {
    ...
    string str ("");
    DefineCodigos(this->raiz, str, h, vet);
    for (int l = 0; l < h; l++) {
        tamanhoCodigo = tamanhoCodigo + (vet[l].getFreq() * vet[l].getHuff().length());
    }
