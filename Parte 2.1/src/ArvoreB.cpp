...
No* buscaAux(int valor, double* comp, No* p)
{
...
void ArvoreB::busca(int valor, double* comp) {
    No* p = buscaAux(valor, comp, this->raiz);
}

No* ArvoreB::BuscaValorParaInsercao(int valor, int* posicao, No* p, bool* result, double* comp)
{
    for(int i = 0 ; i < p->getNumChaves() ; i++)
    {
        (*comp)++;
...
int ArvoreB::achaMediano(No* p, int val, int* posicaoMed, double* comp) {
    int aux;
...
void ArvoreB::separaNo(No* p, No* novoFilhoEsq, No* novoFilhoDir, No* novoNoEsq, No* novoNoDir, int val, int k, double* copias, double* comp) {
    int h = 0;
    int l = 0, c = 0;
    for (l; l < d; l++) {
        if (l == k) {
...
void ArvoreB::splitPropagado(No* p, No* novoFilhoEsq, No* novoFilhoDir, int valor, int k, double* copias, double* comp) {
    int posicao;
...
void ArvoreB::primeiroSplit(No* p, int valor, double* copias, double* comp) {
    No* novoNoEsq = new No();
    No* novoNoDir = new No();
    novoNoDir->setNumChaves(d);
    novoNoEsq->setNumChaves(d);
...
void ArvoreB::inserirValor(int valor, double* copias, double* comp)
{
    if (this->raiz != NULL) {
        bool result = false;
        int posicao = 0;
...
void ArvoreB::remove(int valor, double* comp) {
    int posicao;
    bool result;
    No* p = new No();
