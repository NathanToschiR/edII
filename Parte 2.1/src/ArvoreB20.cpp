...
No* buscaAux20(int valor, double* comp, No* p)
{
    for(int i = 0 ; i < p->getNumChaves() ; i++)
    {
        ...
        if(valor < p->getValor(i))
        {
            if(p->getNo(i) == NULL)
            {
                ...
                return buscaAux20(valor, comp, p->getNo(i));
        }   
    }
    if(p->getNo(p->getNumChaves()) == NULL)
    {
        ...
        return buscaAux20(valor, comp, p->getNo(p->getNumChaves()));
}

void ArvoreB20::busca(int valor, double* comp) {
    No* p = buscaAux20(valor, comp, this->raiz);
}

No* ArvoreB20::BuscaValorParaInsercao(int valor, int* posicao, No* p, bool* result, double* comp)
{
    for(int i = 0 ; i < p->getNumChaves() ; i++)
    {
        (*comp) = (*comp) + 2;
        ...
        if(valor < p->getValor(i))
        {
            if(p->getNo(i) == NULL)
            {
                ...
                *posicao = i;
                ...
                return BuscaValorParaInsercao(valor, posicao, p->getNo(i), result, comp);
        }   
    }
    if(p->getNo(p->getNumChaves()) == NULL)
    {
        ...
        *posicao = p->getNumChaves();
        ...
        return BuscaValorParaInsercao(valor, posicao, p->getNo(p->getNumChaves()), result, comp);
}

No* getMaiorMaisProximo20(int valor, int posicao, No* p, int* i) {
...
int ArvoreB20::achaMediano(No* p, int val, int* posicaoMed, double* comp) {
    int aux;
...
void ArvoreB20::separaNo(No* p, No* novoFilhoEsq, No* novoFilhoDir, No* novoNoEsq, No* novoNoDir, int val, int k, double* copias, double* comp) {
    int h = 0;
    int l = 0, c = 0;
    for (l; l < d; l++) {
        if (l == k) {
...
void ArvoreB20::splitPropagado(No* p, No* novoFilhoEsq, No* novoFilhoDir, int valor, int k, double* copias, double* comp) {
    int posicao;
...
void ArvoreB20::primeiroSplit(No* p, int valor, double* copias, double* comp) {
    No* novoNoEsq = new No();
    No* novoNoDir = new No();
    novoNoDir->setNumChaves(d);
    novoNoEsq->setNumChaves(d);
...
void ArvoreB20::inserirValor(int valor, double* copias, double* comp)
{
    if (this->raiz != NULL) {
        bool result;
        int posicao = 0;
        No* p = BuscaValorParaInsercao(valor, &posicao, this->raiz, &result, comp);
        if (result != true) {
...
//-------------------------------------------------------------- REMOÇÃO - NAO UTILIZADA NO TRABALHO -----------------------------------------------------------

int ArvoreB20::ProcuraNoPai(No* p) {
...
void ArvoreB20::remove(int valor, double* comp) {
    int posicao;
    bool result;
    No* p = new No();
    p = BuscaValorParaInsercao(valor, &posicao, this->raiz, &result, comp);
    if (result == true) {
        if (p->getBool() == false) {
            int novoValor;
            No* ptt = new No();
            ptt = getMaiorMaisProximo20(valor, posicao, p, &novoValor);
...
void ImprimeAux20(No* p, int cont) {
    if (p->getNo(0) != NULL) {
        for (int k = 0; k < p->getNumChaves(); k++) {
            ImprimeAux20(p->getNo(k), cont + 1);
        ...
        ImprimeAux20(p->getNo(p->getNumChaves()), cont + 1);
    }
    else {
        cout << "(";
        for (int k = 0; k < p->getNumChaves(); k++) {
